...
from higgins.http import stream as stream_mod, http, http_headers, responsecode
from higgins.http.channel import http as httpchan
from higgins.http.channel.http import PERSIST_NO_PIPELINE, PERSIST_PIPELINE
from higgins.http.client import interfaces
...
class ClientRequest(object):
    ...
    def __init__(self, method, uri, headers, stream):
        ...
        @type headers: C{dict}, L{higgins.http.http_headers.Headers}, or 
            C{None}
    
        @param stream: Content body to send to the remote HTTP server.
        @type stream: L{higgins.http.stream.IByteStream}
        """
...
class HTTPClientProtocol(basic.LineReceiver, policies.TimeoutMixin, object):
    ...
    def submitRequest(self, request, closeAfter=True):
        ...
        @callback: L{higgins.http.http.Response} from the server.
        """
