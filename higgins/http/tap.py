...
from higgins.http import static, iweb, log, server, channel, vhost
from higgins.http.dav import static as dav_static
...
"""
    ...
    def opt_class(self, className):
        """A class that will be used to serve the root resource.  Must implement higgins.http.iweb.IResource and take no arguments.
        """
        if self['root']:
            raise usage.UsageError("You may only have one root resource.")
